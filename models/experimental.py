...
class ORT_NMS(torch.autograd.Function):
    ...
    def symbolic(g, boxes, scores, max_output_boxes_per_class, iou_threshold, score_threshold):
        return g.op("NonMaxSuppression", boxes, scores, max_output_boxes_per_class, iou_threshold,
                    score_threshold)
...
class ONNX_TRT(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.n_classes == 1:
            ...
            scores *= conf  # conf = obj_conf * cls_conf
        num_det, det_boxes, det_scores, det_classes = TRT_NMS.apply(boxes, scores,
    ...
    def __init__(self, c1, c2, k=(1, 3), s=1, equal_ch=True):
        ...
        if equal_ch:  # equal c_ per group
            ...
            c_ = np.linalg.lstsq(a, b, rcond=None)[
                0].round()  # solve for equal weight indices, ax = b
