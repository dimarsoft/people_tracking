...
def train(hyp, opt, device, tb_writer=None):
    ...
    save_dir, epochs, batch_size, total_batch_size, weights, rank, freeze = \
        Path(
            ...
            opt.weights, opt.global_rank, opt.freeze
    ...
    if pretrained:
        ...
        exclude = ['anchor'] if (opt.cfg or hyp.get('anchors')) \
                                and not opt.resume else []  # exclude keys
    ...
    imgsz, imgsz_test = [check_img_size(x, gs) for x in
                         opt.img_size]  # verify imgsz are gs-multiples
        ...
        if plots:
            ...
            if wandb_logger.wandb:
                ...
                wandb_logger.log({"Results":
                                      [wandb_logger.wandb.Image(str(save_dir / f), caption=f) for f
                                       in files
...
if __name__ == '__main__':
    ...
    if opt.resume and not wandb_run:  # resume an interrupted run
        ...
        opt.cfg, opt.weights, opt.resume, opt.batch_size, opt.global_rank, \
            opt.local_rank = '', ckpt, True, opt.total_batch_size, *apriori  # reinstate
    ...
    # Evolve hyperparameters (optional)
        ...
        for _ in range(300):  # generations to evolve
            if Path('evolve.txt').exists():  # if evolve.txt exists: select best hyps and mutate
                ...
                while all(v == 1):  # mutate until a change occurs (prevent duplicates)
                    v = (g * (npr.random(ng) < mp) * npr.randn(ng) * npr.random() * s + 1) \
                        .clip(0.3, 3.0)
        ...
        print(f'Hyperparameter evolution complete. Best results saved as: {yaml_file}\n'
              ...
              f'$ python train.py --hyp {yaml_file}')
